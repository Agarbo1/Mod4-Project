...
import * as reviewActions from "../../store/reviews";
import { useDispatch } from "react-redux";
import DeleteReviewResultModal from "./DeleteReviewResultModal"; // popup component
...
const ReviewModal = ({ spotId }) => {
  ...
  const [rating, setRating] = useState(0);
  const [errors, setErrors] = useState({});
  const { setModalContent } = useModal(); // only use setModalContent
  ...
  const handleSubmit = async (e) => {
    ...
    if (response?.errors) {
      ...
      setModalContent(
        <DeleteReviewResultModal message="Thank you for your review!" />
      );
      await dispatch(reviewActions.fetchReviews(spotId));
    }
  };
